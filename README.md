...
| `task.Install MODE SRC DST`      | Copy a file with specific permissions; idempotency is content+mode equality (`cmp -s` + `find -perm`), not bare destination existence — editing `SRC` and re-running always re-propagates to `DST` |
